...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_WindowsSetup(false)
{
    ...
    connect(OZWCore::get(), &OZWCore::raiseCriticalError, this,
            &MainWindow::openCriticalDialog);

    OZWCore::get()->initialize();
    ...
    connect(ui->action_Close, SIGNAL(triggered()), this,
            SLOT(CloseConnection()));
    connect(ui->actionDevice_Database, SIGNAL(triggered()), this,
            SLOT(OpenDeviceDB()));
    connect(ui->action_Preferences, SIGNAL(triggered()), this,
            SLOT(openPreferencesWindow()));
    ...
    connect(ui->action_Heal_Network, SIGNAL(triggered()), this,
            SLOT(healNetwork()));
    connect(ui->action_Polling_Config, SIGNAL(triggered()), this,
            SLOT(openPollingConfiguration()));
    connect(this->sbMsg, &statusBarMessages::newMessage, this,
            &MainWindow::setStatusBarMsg);
    connect(OZWCore::get()->getQTOZWManager(),
            &QTOZWManager::remoteConnectionStatus, this,
            &MainWindow::remoteConnectionStatus);
    connect(OZWCore::get()->getQTOZWManager(), &QTOZWManager::readyChanged,
            this, &MainWindow::QTOZW_Ready);
...
void MainWindow::QTOZW_Ready(bool ready)
{
    ...
    for (auto &option : optionlist) {
        qCDebug(ozwadmin) << "Updating Option " << option << " to "
                          << settings.value(option);
        QTOZWOptions *ozwoptions
            = OZWCore::get()->getQTOZWManager()->getOptions();
        QStringList listtypes{"SaveLogLevel", "QueueLogLevel", "DumpLogLevel"};

        if (listtypes.contains(option)) {
            OptionList list = ozwoptions->property(option.toLocal8Bit())
                                  .value<OptionList>();

            list.setSelected(settings.value(option).toString());
        } else {
            ozwoptions->setProperty(option.toLocal8Bit(),
                                    settings.value(option));
        }
    }
    settings.endGroup();
...
void MainWindow::openDefaultWindows()
{
    ...
    if ((OZWCore::get()->getQTOZWManager()->getNodeModel()->rowCount()
         > (int)OZWCore::get()->settings.networkCache())
        || (OZWCore::get()->getQTOZWManager()->getValueModel()->rowCount()
            > (int)OZWCore::get()->settings.networkCache())
        || (OZWCore::get()->getQTOZWManager()->getAssociationModel()->rowCount()
            > (int)OZWCore::get()->settings.networkCache())) {
        quint32 size
            = OZWCore::get()->getQTOZWManager()->getNodeModel()->rowCount();
        size = qMax<int>(
            size,
            OZWCore::get()->getQTOZWManager()->getValueModel()->rowCount());
        size = qMax<int>(size, OZWCore::get()
                                   ->getQTOZWManager()
                                   ->getAssociationModel()
                                   ->rowCount());
        QString msg(
            "Please Increase the Network Cache Size in the Configuration. "
            "Current Setting: %1 Remote Max Records: %2");
        this->openCriticalDialog(
            "Network Cache Size Insufficent",
            msg.arg(OZWCore::get()->settings.networkCache()).arg(size));
    ...
    ValueTable *userValues
        = new ValueTable(QTOZW_ValueIds::ValueIdGenres::User, this);
    ValueTable *systemValues
        = new ValueTable(QTOZW_ValueIds::ValueIdGenres::System, this);
    ValueTable *configValues
        = new ValueTable(QTOZW_ValueIds::ValueIdGenres::Config, this);

    connect(this->ntw, &nodeTableWidget::currentRowChanged, this,
            &MainWindow::NodeSelected);
    connect(this->ntw, &nodeTableWidget::currentRowChanged, di,
            &DeviceInfo::NodeSelected);
    connect(this->ntw, &nodeTableWidget::currentRowChanged, ni,
            &NodeStatus::NodeSelected);
    connect(this->ntw, &nodeTableWidget::refreshNodeInfo,
            this->m_controllerCommands, &ControllerCommands::refreshNodeInfo);
    connect(this->sbMsg, &statusBarMessages::newMessage, ew,
            &EventWindow::newEvent);
    connect(di, &DeviceInfo::openMetaDataWindow, this,
            &MainWindow::openMetaDataWindow);
    ...
    auto RightDockWidget = this->m_DockManager->addDockWidget(
        ads::RightDockWidgetArea, DeviceInfoDW);
    ...
    this->m_DockManager->addDockWidget(ads::CenterDockWidgetArea,
                                       DeviceStatusDW, RightDockWidget);
    ...
    this->m_DockManager->addDockWidget(ads::CenterDockWidgetArea, userValueDW,
                                       RightDockWidget);
    ...
    this->m_DockManager->addDockWidget(ads::CenterDockWidgetArea, systemValueDW,
                                       RightDockWidget);

    ads::CDockWidget *configValueDW = new ads::CDockWidget("Config Values");
    configValueDW->setWidget(configValues);
    /* OSX thinks this is a Preferences Window, so tell QT to tell it "its
     * not!"" */
    ...
    this->m_DockManager->addDockWidget(ads::CenterDockWidgetArea, configValueDW,
                                       RightDockWidget);
    ...
    auto BottomDockWidget = this->m_DockManager->addDockWidget(
        ads::BottomDockWidgetArea, eventViewDW);
    ...
    this->m_DockManager->addDockWidget(ads::CenterDockWidgetArea, logWindowDW,
                                       BottomDockWidget);
    ...
    userValues->setModel(OZWCore::get()->getQTOZWManager()->getValueModel(),
                         this->ntw->selectionModel());
    systemValues->setModel(OZWCore::get()->getQTOZWManager()->getValueModel(),
                           this->ntw->selectionModel());
    configValues->setModel(OZWCore::get()->getQTOZWManager()->getValueModel(),
                           this->ntw->selectionModel());
...
void MainWindow::connectToLocal(QString serial)
{
    ...
    qCDebug(ozwadmin) << "Doing Local Connection: " << serial
                      << QSettings().value("StartServer").toBool();
    startupprogress *sup = new startupprogress(false, this);
    sup->show();
    OZWCore::get()->getQTOZWManager()->initilizeSource(
        QSettings().value("connection/startserver").toBool());
...
void MainWindow::connectToRemote(QUrl server, QString key)
{
    ...
    sup->setWindowFlags(Qt::Window | Qt::WindowTitleHint
...
{
    if (OZWCore::get()->getQTOZWManager()->getConnectionType()
        == ConnectionType::Type::Local) {
        OZWCore::get()->getQTOZWManager()->close();
    } else if (OZWCore::get()->getQTOZWManager()->getConnectionType()
               == ConnectionType::Type::Remote) {
        QMessageBox::critical(this, "Close Connection",
                              "TODO: Please restart the application for now");
        QCoreApplication::exit(0);
    } else {
        QMessageBox::critical(this, "Unknown Connection Type",
...
void MainWindow::openMetaDataWindow()
{
    ...
    quint8 node = model
                      ->data(model->index(index.row(),
                                          QTOZW_Nodes::NodeColumns::NodeID))
                      .value<quint8>();
...
{
    QMessageBox *msgBox = new QMessageBox(QMessageBox::Critical, title, msg,
                                          QMessageBox::Ok, this);
    msgBox->exec();
    return;
}

void MainWindow::addNode() { this->m_controllerCommands->addNode(); }
void MainWindow::delNode() { this->m_controllerCommands->delNode(); }
void MainWindow::healNetwork() { this->m_controllerCommands->healNetwork(); }
...
{
    this->statusBar()->showMessage(QTime::currentTime()
                                       .toString("hh:m:ss ap")
                                       .append(" Event: ")
                                       .append(Msg),
                                   5000);
...
void MainWindow::remoteConnectionStatus(QTOZWManager::connectionStatus status,
                                        QAbstractSocket::SocketError error)
{
    ...
    if (status == QTOZWManager::connectionStatus::ConnectionErrorState) {
        ...
        openCriticalDialog("Connection Error",
                           errorstr.arg(SockErrorToString(error)));
        CloseConnection();
    } else if (status == QTOZWManager::connectionStatus::VersionMisMatchError) {
        openCriticalDialog("Version MisMatch",
                           "The ozwdaemon version is not compatible with this "
                           "version of ozw-admin");
        CloseConnection();
    }
}
